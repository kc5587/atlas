...
**Regression gate (hard stop) — CORRECTED INVARIANT:** the original byte-identical-vs-committed-baseline check is *wrong* here, because the committed `signals.json` baseline predates the `nvidia→oracle` map-expansion edge (first `analysis.leadlag` recompute since it was added) and a SEC fundamentals refresh — both legitimate and independent of networking. The correct invariant is: **the core H1–H10 results must be invariant to the networking feature on identical data.** Verify two things instead: (1) the pipeline's H1 table equals a *networking-absent* recomputation on the same data (`capex_revenue_edges` on fundamentals with ANET/MRVL removed) — max slope diff `0.0`; and (2) **no H0–H10 verdict flips**. If either fails, the partition leaked.

> **Critical subtlety found in execution:** `exclude_stage` partitions nodes/edges but NOT the `fundamentals` frame. H1's leave-one-out cycle factor is built from *every* fundamentals ticker, so adding ANET/MRVL CIKs leaked them into the cycle pool, shifting all 12 core H1 edges (and their q-values/lags) *without* ever appearing in the edge rows. Fix: a `core_fundamentals` helper that drops the excluded stages' tickers, routed to the core scans (leadlag/H1/H5/H2). The networking calls keep the full fundamentals.
